...
public class PListWrapper : Dictionary<string, object>
{
	public PListWrapper()
	{
	}

	public PListWrapper(XElement dict)
	{
		Load(dict);
	}
	...
	private void ParseDictForLoad(PListWrapper dict, IEnumerable<XElement> elements)
	{
	...
	private object ParseValueForLoad(XElement val)
	{
		switch (val.Name.ToString())
		{
			...
			case "false":
				return false;
			case "dict":
				PListWrapper plist = new PListWrapper();
	...
	private XElement ParseDictForSave(PListWrapper dict)
	{
	...
	public XElement ParseValueForSave(Object node)
	{
		...
		else if (node.GetType() == typeof(PListWrapper))
		{
			return ParseDictForSave((PListWrapper)node);
		}
		return null;
	}
